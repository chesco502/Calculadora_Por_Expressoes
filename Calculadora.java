...
// Atividade de Aplicação - APl1- 2023
// feito por:         Francesco zangrandi Coppola TIA: 32271697
       
class Main{
  //primeiro fizemos um repositorio de funçoes que serao utilizadas durante o programa
  
  public static char[] pegaVariavel(String equaçao) { //retorna uma pilha ccom todas a variaveis como letra maiusccula
        int i,I=0;
        char variaveis[] = new char[100];// variaveis podem ser de A a Z
        for(i=0;i<equaçao.length();i++){
            if((int)equaçao.charAt(i)>=65 &&(int)equaçao.charAt(i)<=90 ){//utilizamos tabela ASSCI para a identificaçao
            ...
            }else{// mesma funçao mas para letras minusculas PS:letras minusculas serao consideradas como letras maisculas entao variavel 'A' e 'a' sao a mesma variavel
...
///////////////////////////////////////////////////////
  ...
  private static int precedence(char operator) {//usado para saber a ordem das operaçoes na conversao da prefixa para a pos 
    switch (operator) {
      case '+':
      case '-':
...
//////////////////////////////////////////////////////
  ...
  public static boolean eVariavel(int n){// usado para verificar se um elemento da operaçao é uma variavel
    if((int)n>=65 &&(int)n<=90 ){// faz check usando tabela ASSCI se valor estiver entre 65(A) ate 90(Z) retorna true 
      return true;
    }else{
...
public static String infixaPosfixa(String infix) {// funçao de conversao de equaçao da ordem infixa para posfixa
  ...
  for (int i = 0; i < infix.length(); i++) {
    ...
    // Se c é um operador, desempilha todos os operadores de maior ou igual precedência e adiciona ao resultado posfixo
    else {
  ...
  // Desempilhando todos os operadores restantes na pilha e adicionando ao resultado posfixo
  while (!pilha.isEmpty()) {
    postfix.append((char)pilha.pop());
  }
...
public static boolean contains(char test[], char V){// teste para verificar se um array de char contain alguma variavel
...
////////////////////////////////////////////////
  ...
  public static int calcula(String posfixa,char[] variaveis,int[] valorVariaveis){//funçao calculadora
    ...
    while(equaçao.isEmpty()==false){//enquanto a equaçao nao for totalmente calculada
      while(equaçao.isEmpty()==false&&!contains(operadores,(char)equaçao.peek())){// teste para ver se proximo é um operador 
      ...
      if(equaçao.isEmpty()==false){// teste para garantir que a pilha n esta vazia 
        ...
        if(operador=='+'){// operaçao de mais 
          // como a pilha esta estocando somente variaveis e os valores de tais variaveis estao estocadas em vetor tivemos que dar uma improvisada
          N=substituir(A,variaveis,valorVariaveis)+substituir(B,variaveis,valorVariaveis); // substuir muda de Variavel como 'A' para seu valor como 12 e faz a operaçao
          temp=adicionar(variaveis,valorVariaveis);// para podermos jogar o valor de volta a pillha temos que saber que variavel ainda nao foi alocada e onde poder alocar isso que adicionar retorna 
        ...
        if(operador=='^'){// operaçao de ^
          // o mesmo conceito somente mudando a operaçao
          N= pow(substituir(A,variaveis,valorVariaveis),substituir(B,variaveis,valorVariaveis));// tivemos que fazer uma funçao pow por conta de estarmos usando int n double 
          temp=adicionar(variaveis,valorVariaveis);
          variaveis[temp[1]]=(char)temp[0];
          valorVariaveis[temp[1]]=N;
          equaçao.push((char)temp[0]);
...
////////////////////////////////////////////////
  ...
  public static int substituir(char variavel,char[] variaveis,int[] valorVariaveis){//recebe a variavel e retorna seu valor
    int i=0;
...
////////////////////////////////////////////////
  ...
  public static int[] adicionar(char[] variaveis, int[] valorVariaveis ){//retorna qual variavel ainda nao esta alocada e onde pode ser alocada 
...
public static boolean validaçao(String afixa){ // funçao de validaçao
  ...
  if(parenteses==0&&variavel==operador+1){// teste de valides 
    return true;
  }else{//se parenteses nao forem fechados/abertos e se nao ouver a quantidade certa de operandos para variaveis 
    return false;
  }
}
////////////////////////////////////////////////
